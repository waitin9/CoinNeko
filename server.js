...
// Seed default data (cat_species、categories 等保持原本內容不動，僅略過中間長陣列以節省篇幅)
...
// 不再預先塞一個預設 user，改成每個 userId 第一次來時才自動建立

// ===== 共用：取得或建立使用者 =====
function getOrCreateUser(userId) {
  if (!userId) {
    const newId = uuidv4();
    db.prepare('INSERT INTO users (id, username, coins, gacha_tickets) VALUES (?,?,?,?)')
      .run(newId, '喵喵理財師', 0, 1); // 第一次登入就給 1 張扭蛋券
    ensureTodayLoginReward(newId);
  ...
  if (!user) {
    db.prepare('INSERT INTO users (id, username, coins, gacha_tickets) VALUES (?,?,?,?)')
      .run(userId, '喵喵理財師', 0, 1); // 新 userId 自動註冊 + 1 張券
    ensureTodayLoginReward(userId);
...
// 每日登入送一次抽（每天 1 張券）
function ensureTodayLoginReward(userId) {
  const today = new Date().toISOString().split('T')[0];
  const existing = db.prepare('SELECT * FROM daily_checkins WHERE user_id = ? AND checkin_date = ?').get(userId, today);
  if (!existing) {
    db.prepare('INSERT INTO daily_checkins (id, user_id, checkin_date, transactions_count) VALUES (?,?,?,0)')
      .run(uuidv4(), userId, today);
    db.prepare('UPDATE users SET gacha_tickets = gacha_tickets + 1 WHERE id = ?').run(userId);
  }
}

// 取得 request userId（從 header x-user-id）
...
app.post('/api/transactions', (req, res) => {
  ...
  db.prepare('INSERT INTO transactions (id, user_id, category_id, amount, note) VALUES (?,?,?,?,?)')
    .run(id, user.id, category_id, amount, note || '');

  // 記帳給貓咪幣
  const coinsEarned = 10;
  db.prepare('UPDATE users SET coins = coins + ? WHERE id = ?').run(coinsEarned, user.id);

  // 每天第一次記帳再多送 1 張券（維持你原本 daily_checkins 的邏輯）
  const today = new Date().toISOString().split('T')[0];
  const existing = db.prepare('SELECT * FROM daily_checkins WHERE user_id = ? AND checkin_date = ?').get(user.id, today);
  let ticketEarned = 0;
  if (!existing) {
    db.prepare('INSERT INTO daily_checkins (id, user_id, checkin_date, transactions_count) VALUES (?,?,?,1)')
      .run(uuidv4(), user.id, today);
    ticketEarned = 1;
    db.prepare('UPDATE users SET gacha_tickets = gacha_tickets + 1 WHERE id = ?').run(user.id);
  } else {
    db.prepare('UPDATE daily_checkins SET transactions_count = transactions_count + 1 WHERE user_id = ? AND checkin_date = ?')
      .run(user.id, today);
  }
...
app.get('/api/summary', (req, res) => {
  ...
  const income = db.prepare(`
    ...
    FROM transactions t JOIN categories c ON t.category_id = c.id
    WHERE t.user_id = ? AND c.type='income' AND strftime('%Y-%m', t.transacted_at) = ?
  `).get(user.id, m);

  const expense = db.prepare(`
    SELECT COALESCE(SUM(t.amount),0) as total
    FROM transactions t JOIN categories c ON t.category_id = c.id
    WHERE t.user_id = ? AND c.type='expense' AND strftime('%Y-%m', t.transacted_at) = ?
  `).get(user.id, m);
...
app.get('/api/budgets', (req, res) => {
  ...
  const budgets = db.prepare(`
    ...
    COALESCE((
      ...
      AND strftime('%Y-%m', t.transacted_at) = b.month
    ), 0) as spent
...
app.post('/api/budgets', (req, res) => {
  ...
  const id = uuidv4();
  try {
    db.prepare('INSERT OR REPLACE INTO budgets (id, user_id, category_id, month, limit_amount) VALUES (?,?,?,?,?)')
      .run(id, user.id, category_id, month, limit_amount);
    res.json({ success: true });
  } catch (e) {
...
app.get('/api/cats/collection', (req, res) => {
  ...
  const collected = db.prepare(`
    SELECT uc.*, cs.name, cs.job_title, cs.rarity, cs.emoji, cs.description
...
app.post('/api/gacha/pull', (req, res) => {
  ...
  if (existing) {
    ...
    db.prepare('INSERT INTO user_cats (id, user_id, cat_species_id) VALUES (?,?,?)')
      .run(uuidv4(), user.id, picked.id);
  }

  db.prepare('INSERT INTO gacha_history (id, user_id, cat_species_id) VALUES (?,?,?)')
    .run(uuidv4(), user.id, picked.id);
